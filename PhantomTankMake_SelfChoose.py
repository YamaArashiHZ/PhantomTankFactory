...
def resize_image(surface, inner):
    ...
    resized_width = int(ratio * inner_width)
    resized_height = int(ratio * inner_height)
...
def brighten_image(image, lightness):
    ...
    brightened_image = Image.merge(
        'RGBA',
        (brightened_image.split()[0], brightened_image.split()[1], brightened_image.split()[2], a)
    )
...
def invert_image(image):
    ...
    inverted_image = Image.merge(
        'RGBA',
        (inverted_image.split()[0], inverted_image.split()[1], inverted_image.split()[2], a)
    )
...
def config_verify(config_detail: str, config_value):
    ...
    if config_detail == 'brightness_enhancement':
        try:
            ...
            return False
    elif config_detail == 'brightness_reduction':
        try:
            num = float(config_value)
            if -100 <= num <= 0:
...
debug_mode: False'''
    ...
    try:
        ...
        logger.info('正在调整灰度...')
        gray_surface = desaturate_image_with_alpha(resized_surface_image)
        gray_inner = desaturate_image_with_alpha(resized_inner_image)
